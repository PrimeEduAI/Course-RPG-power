...
# cape z-band weights (heat weighting fails on cape)
...
if not any(m.type == 'ARMATURE' for m in cape.modifiers):
    cape.modifiers.new("Armature", 'ARMATURE').object = rig
cape.parent = rig
BANDS = [("Chest", 1.50), ("CapeRoot", 1.33), ("Cape_1", 1.10), ("Cape_2", 0.80), ("Cape_3", 0.35)]
groups = {b: cape.vertex_groups.new(name=b) for b, _ in BANDS}
names = [b for b, _ in BANDS]
...
for v in cape.data.vertices:
    ...
    for i in range(len(centers) - 1):
        ...
        if c1 <= z <= c0:
            t = (z - c1) / (c0 - c1)
            if t > 0: groups[names[i]].add([v.index], t, 'REPLACE')
            if t < 1: groups[names[i + 1]].add([v.index], 1 - t, 'REPLACE')
            break

# eye patch verts -> rigid Head
body = bpy.data.objects["Body"]
eye_slot = next((i for i, m in enumerate(body.data.materials) if m and m.name == "M_Eyes"), None)
if eye_slot is not None:
    eye_verts = set()
    for poly in body.data.polygons:
        if poly.material_index == eye_slot:
    ...
    hg.add(list(eye_verts), 1.0, 'REPLACE')
    print("eye verts -> Head:", len(eye_verts))
